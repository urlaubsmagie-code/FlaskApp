...
5. Add authorized redirect URI: http://localhost/chatbot/gmail/callback
...
class GmailService:
    ...
    def get_recent_emails(self, max_results: int = 10, query: str = None,
        ...
        try:
            ...
            for msg in messages:
                ...
                if email_data:
                    # Apply vacation rental filter
                    if apply_filter and not self.email_matches_filter(email_data):
                        filtered_count += 1
                        logger.debug(f"Filtered out email from {email_data.get('sender_email')}: "
                                    f"'{email_data.get('subject', '')[:50]}'")
                        continue
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'email': self.get_user_email() if self.is_authenticated() else None
        }
