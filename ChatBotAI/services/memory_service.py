...
class MemoryService:
    ...
    def _store_extracted_info(self, guest_id: int, extracted: Dict[str, Any], message_id: int) -> None:
        ...
        # Store booking details (num_guests, check_in, check_out)
        if extracted.get('num_guests'):
            self.store_detail(
                guest_id=guest_id,
                detail_type='booking',
                detail_key='num_guests',
                detail_value=str(extracted['num_guests']),
                source_message_id=message_id,
                confidence=0.9
            )
        ...
        if extracted.get('check_in'):
            self.store_detail(
                ...
                detail_type='booking',
                detail_key='check_in',
                detail_value=str(extracted['check_in']),
                source_message_id=message_id,
                confidence=0.9
            )
        ...
        if extracted.get('check_out'):
            self.store_detail(
                guest_id=guest_id,
                detail_type='booking',
                detail_key='check_out',
                detail_value=str(extracted['check_out']),
                source_message_id=message_id,
                confidence=0.9
            )
    ...
    def store_detail(
        ...
        # Detail types where only the latest value matters (one value per key)
        REPLACE_TYPES = {'booking', 'language'}

        try:
            if detail_type in REPLACE_TYPES:
                # For booking/language: update existing record if key exists
                ...
                if existing:
                    if existing.detail_value == detail_value:
                        ...
                        return False
                    old_value = existing.detail_value
                    existing.detail_value = detail_value
                    existing.source_message_id = source_message_id
                    existing.confidence = confidence
                    existing.extracted_at = datetime.utcnow()
                    db.session.commit()
                    logger.info(f"Updated guest detail: {detail_type}:{detail_key} '{old_value}' -> '{detail_value}' for guest {guest_id}")
                    return True
            else:
                # For accumulating types: check for exact duplicate
                existing = GuestDetail.query.filter_by(
                    guest_id=guest_id,
                    detail_type=detail_type,
                    detail_key=detail_key,
    ...
    def get_guest_memory_summary(self, guest_id: int) -> str:
        ...
        if profile.get('booking'):
            booking_map = {b['key']: b['value'] for b in profile['booking']}
            booking_parts = []
            if booking_map.get('num_guests'):
                booking_parts.append(f"{booking_map['num_guests']} guests")
            if booking_map.get('check_in'):
                booking_parts.append(f"check-in: {booking_map['check_in']}")
            if booking_map.get('check_out'):
                booking_parts.append(f"check-out: {booking_map['check_out']}")
            if booking_parts:
                parts.append(f"Booking: {', '.join(booking_parts)}")
